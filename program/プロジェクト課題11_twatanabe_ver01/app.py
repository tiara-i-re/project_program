...
teams = ["▼選択してください", "G大阪", "C大阪", "浦和レッズ", "鹿島アントラーズ", "川崎フロンターレ", "FC東京",
         "名古屋グランパス", "柏レイソル", "サンフレチェ広島","横浜FM", "ヴィッセル神戸", "北海道コンサドーレ札幌",
...
def predict():
    ...
    if request.method == 'POST':
        ...
        #pred_class = request.form['class']

        p = prediction.Prediction(hometeam, awayteam)
        p.print_team_name()
...
def result_print():
    message = ['accuracy score is...', 'あなたの予測結果は', 'ホームチーム：', 'アウェイチーム：']
    if request.method == 'POST':

        # accuracyなどの値を他のリクエストから受け取り．
